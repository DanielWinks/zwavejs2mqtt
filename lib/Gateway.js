...
function onValueChanged (valueId, node, changed) {
  ...
  switch (this.config.payloadType) {
    case 1: // entire zwave valueId object
      if (valueId.commandClass === CommandClasses['Scene Activation']
      && valueId.hasOwnProperty('states')
      && valueId.property === 'scene') {
        data = tmpVal
      } else {
        data = copy(valueId)
        data.value = tmpVal
  ...
  try {
    ...
    switch (cmdClass) {
      ...
      case CommandClasses['Scene Activation']:
        if (valueId.hasOwnProperty('states') && valueId.property === 'scene') {
          for (const state of valueId.states) {
            cfg = copy(hassCfg.device_trigger)
            ...
            cfg.discovery_payload.topic = getTopic
            ...
            switch (state.text) {
              case 'KeyPressed':
                ...
                triggerPayload = state.value
                break
              case 'KeyReleased':
                triggerType = 'button_long_release'
                triggerPayload = state.value
                break
              case 'KeyHeldDown':
                triggerType = 'button_long_press'
                triggerPayload = state.value
                break
              case 'KeyPressed2x':
                triggerType = 'button_double_press'
                triggerPayload = state.value
                break
              case 'KeyPressed3x':
                triggerType = 'button_triple_press'
                triggerPayload = state.value
                break
              case 'KeyPressed4x':
                triggerType = 'button_quadruple_press'
                triggerPayload = state.value
                break
              case 'KeyPressed5x':
                triggerType = 'button_quintuple_press'
                triggerPayload = state.value
                break
              default:
                break
            }
            cfg.discovery_payload.type = triggerType
            cfg.discovery_payload.subtype = valueId.propertyKey
            cfg.discovery_payload.payload = triggerPayload
            cfg.discovery_payload.value_template = '{{ value_json.value|int }}'
